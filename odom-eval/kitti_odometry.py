...
def scale_solver_plain(X, Y):
    ...
    for i in range(len(X)):
        # if np.linalg.norm(Y[i]) < 0.01:
        #     scale.append(0)
        # else:
        scale.append((np.linalg.norm(Y[i])+eps)/(np.linalg.norm(X[i])+eps))
    return scale
...
class KittiEvalOdom():
    ...
    def compute_ASE(self, gt, pred):
        ...
        for i in list(pred.keys())[:-1]:
            ...
            # print(np.sqrt(gt_rel_trans[0]**2+gt_rel_trans[1]**2+gt_rel_trans[2]**2))
            ...
            pred_rel_trans_norm = np.linalg.norm(pred_rel_trans)

            scale_error = 1 - min((gt_rel_trans_norm+eps)/(pred_rel_trans_norm+eps), (pred_rel_trans_norm+eps)/(gt_rel_trans_norm+eps))

            if gt_rel_trans_norm <= 0.01:
                pass
            else:
        ...
        if len(scale_errors) == 0:
            ...
            ase = np.mean(np.asarray(scale_errors))
            return ase
    ...
    def scale_optimization_plain(self, gt, pred):
        """ Optimize scaling factor
        Args:
            gt (4x4 array dict): ground-truth poses
            pred (4x4 array dict): predicted poses
        ...
        xyz_pred = np.asarray(xyz_pred)
        xyz_ref = np.asarray(xyz_ref)
        scale = scale_solver_plain(xyz_pred, xyz_ref)
        for i in pred_updated:
            pred_updated[i][:3, 3] *= scale[i]
        return pred_updated
    ...
    def eval(self, gt_dir, result_dir, 
        ...
        """
        dataset = gt_dir.split('/')[-3]
        seq_dir = sorted(glob(os.path.join(gt_dir, "*.txt")))
        seq_list = [i.split('/')[-1].split('.')[0] for i in seq_dir]
        ...
        if seqs is None:
            ...
            self.eval_seqs = [i.split('/')[-1].split('.')[0] for i in available_seqs if i.split('/')[-1].split('.')[0] in seq_list]
        ...
        for i in self.eval_seqs:
            ...
            if alignment == "plain":
                poses_result = self.scale_optimization_plain(poses_gt, poses_result)
