...
class KittiEvalOdom():
    ...
    def eval(self, gt_dir, result_dir, 
        ...
        """
        scene = gt_dir.split('/')[-3]

        # seq_list = ["{:02}".format(i) for i in range(0, 11)]
        seq_dir = glob(os.path.join(gt_dir, "*.txt"))
        ...
        if seqs is None:
            ...
            self.eval_seqs = [int(i.split('/')[-1].split('.')[0]) for i in available_seqs if i.split('/')[-1].split('.')[0] in seq_list]
        else:
            self.eval_seqs = seqs
        ...
        for i in self.eval_seqs:
            ...
            if scene == 'kitti':
                ...
                self.cur_seq = '{:03}'.format(i)
                file_name = '{:03}.txt'.format(i)
            elif scene == 'argo2':
                self.cur_seq = '{:03}'.format(i)
                file_name = '{:03}.txt'.format(i)
            ...
            dict_t[i].append(ave_t_err*100)
            dict_r[i].append(ave_r_err/np.pi*180*100)
            dict_a[i].append(ate)
            dict_s[i].append(ase)
        ...
        return pd.DataFrame(dict_t), pd.DataFrame(dict_r), pd.DataFrame(dict_a), pd.DataFrame(dict_s)
